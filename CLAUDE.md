...
- `descriptors` — a freeform `key → string` object, with two exceptions: `medium` and `author` are `key → string[]`, since a Representational entry can be more than one medium at once (e.g. a Drawing that's also a Photograph) and a piece can have more than one credited author — every other choice field (`primative`, `status`) stays single-select/string. See `MULTI_CHOICE_KEYS`/`MULTI_AUTOCOMPLETE_KEYS` in `NewEntryPage.jsx` and the array-handling branch in `cleanDescriptors()`/`matchesFilter()`. The set of available keys is **data, not code**: stored in a single Firestore config doc (`config/descriptorFields`, shape `{ fields: [{key,label}] }`), seeded/merged on every allowed sign-in via `ensureDescriptorFieldsSeeded()` (`src/data/descriptorFields.js`), and extendable at runtime with `addDescriptorField(key, label)` — no deploy needed. Current default keys: `primative` (Physical / Representational / Discursive — the one descriptor rendered as a radio group instead of free text, in `NewEntryPage.jsx`), `author`, `collaborator`, `year`, `medium`, `project`, `location`, `status`, `source`, `country`. Publication/Source (`source`) is only collected for Discursive entries — dropped from the form for Physical/Representational, and hidden on the entry page unless `primative === "Discursive"`; it's a plain text field (e.g. a publication name), not a URL. Author and Project use the same live search/create dropdown as Tags (`CreatableSelect`), prepopulated from values already used elsewhere — Author allows multiple selections (`multiple`), Project stays single. Only non-empty values are persisted (`cleanDescriptors()` in `src/data/entries.js`).
...
A separate, deliberately minimal top-level collection for lightweight, uncategorized image uploads — things worth keeping around without the overhead of the full Entry form. Each doc: `{ title, tags: [...], image: { url, path }, createdAt }` — no `postedBy`, no descriptors, always exactly one image (never an array). CRUD lives in `src/data/flotsam.js`; `deleteFlotsam()` removes both the Firestore doc and the Storage file, while `deleteFlotsamRecord()` removes only the doc — used by the "Convert to Entry" flow (`FlotsamDetailPage.jsx` → `NewEntryPage.jsx` via `/new?fromFlotsam=<id>`), which hands the same Storage image off to the new entry doc rather than re-uploading it, so only the flotsam record itself should disappear. `FlotsamPage.jsx` is the grid at `/flotsam`, styled as an inverted (black/white) 4-column view via the `.page-flotsam` class in `index.css` — see the comment there for how the invert works (overriding `--ink`/`--bg` rather than restyling every element individually). **`flotsam` needs its own Firestore and Storage security rules (see README.md) — it won't work until those are added in Firebase Console.**
